...
const getDoctor =async()=>{
    await axios.get("http://localhost:4000/getDocto").then((response)=>{
...
const getPatient =async()=>{
    await axios.get("http://localhost:4000/getPatients").then((response)=>{
  ...
  const getDoctorTimeSlots = async (Username) => {
    try {
      const response = await axios.get(`http://localhost:4000/getTimeSlots/${Username}`);
    ...
    if(reservationType === 'family')
     {
        if(!selectedFamilyMember){
            await axios.post("http://localhost:4000/reserveTimeSlot", {Dusername:doctor.Username,Pusername:patient.Username,Did:doctor._id,Pid:patient._id,PFamilyMemberName:familyMembers[0]?.Name,PFamilyMemberID:familyMembers[0]?.ID,ADate:selectedSlot}).then((response)=>{
        ...
        setLoading(false)
            ...
            await axios.post("http://localhost:4000/reserveTimeSlot", {Dusername:doctor.Username,Pusername:patient.Username,Did:doctor._id,Pid:patient._id,PFamilyMemberName:familyMember.Name,PFamilyMemberID:familyMember.ID,ADate:selectedSlot}).then((response)=>{
        console.log(response.data);
        setLoading(false)
      }).catch((err)=>{
        console.log(err)
        ...
        await axios.post("http://localhost:4000/reserveTimeSlot", {Dusername:doctor.Username,Pusername:patient.Username,Did:doctor._id,Pid:patient._id,ADate:selectedSlot}).then((response)=>{
        console.log(response.data);
        setLoading(false)
      }).catch((err)=>{
        console.log(err)
