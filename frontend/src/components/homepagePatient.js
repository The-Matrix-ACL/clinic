  ...
  return (
    <>
      ...
      <div className='homepage' style={{ display: 'flex', justifyContent: 'space-around', padding: '10px' }}>
        ...
        <><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/emaildoctor?docid=${userId}`}>Notifications</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/ListDoctors?docid=${userId}`}>view all Doctors</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/viewappointments?docid=${userId}`}>View Appointments</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/addhealthrecords`}>Add Health Records</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/removehealthrecords`}>view Health Records</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/addFamilyMember`}>Add Family Member</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/updatepasswordpatient`}>Change Password</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/viewWallet`}>View Wallet</button><a href="http://localhost:3000/">Logout</a></>
