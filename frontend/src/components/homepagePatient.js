  ...
  return (
    <>
      ...
      <div className='homepage' style={{ display: 'flex', justifyContent: 'space-around', padding: '10px' }}>
        ...
        <><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/emaildoctor?docid=${userId}`}>Notifications</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/ListDoctors?docid=${userId}`}>view all Doctors</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/viewappointments?docid=${userId}`}>View Appointments</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/patientpre?docid=${userId}`}>View Prescriptions</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/addhealthrecords?docid=${userId}`}>Add Health Records</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/removehealthrecords`}>view Health Records</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/addFamilyMember?docid=${userId}`}>Add Family Member</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/updatepasswordpatient?docid=${userId}`}>Change Password</button><button type="submit" style={{ backgroundColor: '#4584ff'}} onClick={() => window.location.href = `/viewWallet?docid=${userId}`}>View Wallet</button><a href="http://localhost:3000/">Logout</a></>
